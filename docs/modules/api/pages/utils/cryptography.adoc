 ...
 * {SignerAccessManaged}: Implementation of an https://docs.openzeppelin.com/contracts/5.x/api/utils/cryptography#AbstractSigner[AbstractSigner] whose authority is delegated to the members of a role tracked by an `IAccessManager`.
...
:constructor: pass:normal[xref:#SignerAccessManaged-constructor-address-uint64-[`++constructor++`]]
:accessManager: pass:normal[xref:#SignerAccessManaged-accessManager--[`++accessManager++`]]
:roleId: pass:normal[xref:#SignerAccessManaged-roleId--[`++roleId++`]]
:_isAuthorizedMember: pass:normal[xref:#SignerAccessManaged-_isAuthorizedMember-address-[`++_isAuthorizedMember++`]]
:_rawSignatureValidation: pass:normal[xref:#SignerAccessManaged-_rawSignatureValidation-bytes32-bytes-[`++_rawSignatureValidation++`]]

[.contract]
[[SignerAccessManaged]]
=== `++SignerAccessManaged++` link:https://github.com/OpenZeppelin/openzeppelin-community-contracts/blob/master/contracts/utils/cryptography/signers/SignerAccessManaged.sol[{github-icon},role=heading-link]

[.hljs-theme-light.nopadding]
```solidity
import "@openzeppelin/community-contracts/utils/cryptography/signers/SignerAccessManaged.sol";
```

Implementation of {AbstractSigner} whose authority is delegated to the members of a role
...
.Functions
--
* `++constructor(accessManager_, roleId_)++`
* `++accessManager()++`
* `++roleId()++`
...
[.contract-item]
[[SignerAccessManaged-constructor-address-uint64-]]
==== `[.contract-item-name]#++constructor++#++(address accessManager_, uint64 roleId_)++` [.item-kind]#internal#

Create a new signer where access is managed by the given access manager and role id.
Access is checked via {_isAuthorizedMember}.

[.contract-item]
[[SignerAccessManaged-accessManager--]]
==== `[.contract-item-name]#++accessManager++#++() → contract IAccessManager++` [.item-kind]#public#

Returns the {IAccessManager} whose role membership authorizes signatures for this signer.

[.contract-item]
[[SignerAccessManaged-roleId--]]
==== `[.contract-item-name]#++roleId++#++() → uint64++` [.item-kind]#public#

Returns the role id this signer is bound to. Members of this role in the {accessManager}
are authorized to produce signatures on behalf of this signer.

[.contract-item]
[[SignerAccessManaged-_isAuthorizedMember-address-]]
==== `[.contract-item-name]#++_isAuthorizedMember++#++(address account) → bool++` [.item-kind]#internal#

Returns whether `account` currently holds {roleId} in the {accessManager} and has no execution delay.
This signer does not allow roles with execution delays to interact with it.

[.contract-item]
[[SignerAccessManaged-_rawSignatureValidation-bytes32-bytes-]]
==== `[.contract-item-name]#++_rawSignatureValidation++#++(bytes32 hash, bytes signature) → bool++` [.item-kind]#internal#
