...
import {ZKEmailUtils} from "@openzeppelin/community-contracts/contracts/utils/cryptography/ZKEmailUtils.sol";
 ...
 */
abstract contract SignerZKEmailUpgradeable is Initializable, AbstractSigner {
    using ZKEmailUtils for EmailProof;

    /// @custom:storage-location erc7201:openzeppelin.storage.SignerZKEmail
    struct SignerZKEmailStorage {
        bytes32 _accountSalt;
        IDKIMRegistry _registry;
        IGroth16Verifier _groth16Verifier;
    }

    // keccak256(abi.encode(uint256(keccak256("openzeppelin.storage.SignerZKEmail")) - 1)) & ~bytes32(uint256(0xff))
    bytes32 private constant SignerZKEmailStorageLocation = 0x8cd60073568df4b28ff9ea9a394e1ad192683a0b613125b2e915ce1a974e7300;

    function _getSignerZKEmailStorage() private pure returns (SignerZKEmailStorage storage $) {
        assembly {
            $.slot := SignerZKEmailStorageLocation
        }
    }
    ...
    function __SignerZKEmail_init() internal onlyInitializing {
    }

    function __SignerZKEmail_init_unchained() internal onlyInitializing {
    }
    /**
    ...
    function accountSalt() public view virtual returns (bytes32) {
        SignerZKEmailStorage storage $ = _getSignerZKEmailStorage();
        return $._accountSalt;
    }
