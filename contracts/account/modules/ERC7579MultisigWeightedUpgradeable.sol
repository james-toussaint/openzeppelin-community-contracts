 ...
 */
abstract contract ERC7579MultisigWeightedUpgradeable is Initializable, ERC7579MultisigUpgradeable {
    using EnumerableSet for EnumerableSet.BytesSet;
    using SafeCast for *;

    /// @custom:storage-location erc7201:openzeppelin.storage.ERC7579MultisigWeighted
    struct ERC7579MultisigWeightedStorage {
        // Sum of all the extra weights of all signers. Each signer has a base weight of 1.
        mapping(address account => uint64 totalExtraWeight) _totalExtraWeight;

        // Mapping from account => signer => extraWeight (in addition to all authorized signers having weight 1)
        mapping(address account => mapping(bytes signer => uint64)) _extraWeights;
    }

    // keccak256(abi.encode(uint256(keccak256("openzeppelin.storage.ERC7579MultisigWeighted")) - 1)) & ~bytes32(uint256(0xff))
    bytes32 private constant ERC7579MultisigWeightedStorageLocation = 0x073db3c61562b076e3c7bad98f2abd9848cd44f9eac5465723a17f38904b2100;

    function _getERC7579MultisigWeightedStorage() private pure returns (ERC7579MultisigWeightedStorage storage $) {
        assembly {
            $.slot := ERC7579MultisigWeightedStorageLocation
        }
    }
    ...
    function __ERC7579MultisigWeighted_init() internal onlyInitializing {
    }

    function __ERC7579MultisigWeighted_init_unchained() internal onlyInitializing {
    }
    /**
    ...
    function onUninstall(bytes calldata data) public virtual override {
        ERC7579MultisigWeightedStorage storage $ = _getERC7579MultisigWeightedStorage();
