 ...
 */
abstract contract ERC7579DelayedExecutorUpgradeable is Initializable, ERC7579ExecutorUpgradeable {
    using Time for *;
    ...
    /// @custom:storage-location erc7201:openzeppelin.storage.ERC7579DelayedExecutor
    struct ERC7579DelayedExecutorStorage {
        mapping(address account => ExecutionConfig) _config;
        mapping(bytes32 operationId => Schedule) _schedules;
    }

    // keccak256(abi.encode(uint256(keccak256("openzeppelin.storage.ERC7579DelayedExecutor")) - 1)) & ~bytes32(uint256(0xff))
    bytes32 private constant ERC7579DelayedExecutorStorageLocation = 0x778314987e2bb6640d41055fa1c386e03ed7911a22a8c706d19fba6494814800;

    function _getERC7579DelayedExecutorStorage() private pure returns (ERC7579DelayedExecutorStorage storage $) {
        assembly {
            $.slot := ERC7579DelayedExecutorStorageLocation
        }
    }

    function __ERC7579DelayedExecutor_init() internal onlyInitializing {
    }

    function __ERC7579DelayedExecutor_init_unchained() internal onlyInitializing {
    }
    ...
    function state(bytes32 operationId) public view returns (OperationState) {
        ERC7579DelayedExecutorStorage storage $ = _getERC7579DelayedExecutorStorage();
