 ...
 * backup.
 */
contract ERC7579SignatureUpgradeable is Initializable, ERC7579ValidatorUpgradeable {
    /// @custom:storage-location erc7201:openzeppelin.storage.ERC7579Signature
    struct ERC7579SignatureStorage {
        mapping(address account => bytes signer) _signers;
    }

    // keccak256(abi.encode(uint256(keccak256("openzeppelin.storage.ERC7579Signature")) - 1)) & ~bytes32(uint256(0xff))
    bytes32 private constant ERC7579SignatureStorageLocation = 0xcd34a6ffa7fde347a4ae71271cb4874a2da81c5bdcf0fa2ff0ec41c1cc378700;

    function _getERC7579SignatureStorage() private pure returns (ERC7579SignatureStorage storage $) {
        assembly {
            $.slot := ERC7579SignatureStorageLocation
        }
    }
    ...
    function __ERC7579Signature_init() internal onlyInitializing {
    }

    function __ERC7579Signature_init_unchained() internal onlyInitializing {
    }
    /// @dev Return the ERC-7913 signer (i.e. `verifier || key`).
    function signer(address account) public view virtual returns (bytes memory) {
        ERC7579SignatureStorage storage $ = _getERC7579SignatureStorage();
