...
import {AxelarExecutableUpgradeable} from "../../vendor/@axelar-network/axelar-gmp-sdk-solidity/contracts/executable/AxelarExecutableUpgradeable.sol";
...
contract AxelarGatewayAdapterUpgradeable is Initializable, IERC7786GatewaySource, OwnableUpgradeable, AxelarExecutableUpgradeable {
    using InteroperableAddress for bytes;
    using Strings for *;

    /// @custom:storage-location erc7201:openzeppelin.storage.AxelarGatewayAdapter
    struct AxelarGatewayAdapterStorage {
        ...
        mapping(bytes erc7930 => string axelar) _erc7930ToAxelar;
        mapping(string axelar => bytes erc7930) _axelarToErc7930;
    }

    // keccak256(abi.encode(uint256(keccak256("openzeppelin.storage.AxelarGatewayAdapter")) - 1)) & ~bytes32(uint256(0xff))
    bytes32 private constant AxelarGatewayAdapterStorageLocation = 0x704ea0b2474a3cbc1fde25f5dabd6a199ef73ad40fc7b5b32d256c6d32993b00;

    function _getAxelarGatewayAdapterStorage() private pure returns (AxelarGatewayAdapterStorage storage $) {
        assembly {
            $.slot := AxelarGatewayAdapterStorageLocation
        }
    }
    ...
    function __AxelarGatewayAdapter_init(
        IAxelarGateway gateway,
        address initialOwner
    ) internal onlyInitializing {
        __Ownable_init_unchained(initialOwner);
        __AxelarExecutable_init_unchained(address(gateway));
    }

    function __AxelarGatewayAdapter_init_unchained(
        IAxelarGateway,
        address
    ) internal onlyInitializing {}
    ...
    function getAxelarChain(bytes memory input) public view virtual returns (string memory output) {
        AxelarGatewayAdapterStorage storage $ = _getAxelarGatewayAdapterStorage();
